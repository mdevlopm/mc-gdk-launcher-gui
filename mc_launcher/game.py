    ...
    def worker():
        ...
        for root_dir in search_roots:
            if not os.path.exists(root_dir):
                continue
            try:
                cmd = ["find", root_dir, "-maxdepth", "4", "-type", "f", "-name", "Minecraft.Windows.exe"]
                proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
                stdout, _ = proc.communicate(timeout=10)
                out = stdout.decode("utf-8").strip()
                if out:
                    found.extend(p.strip() for p in out.split("\n") if p.strip())
            except Exception as e:
...
def stop_game(proc) -> None:
    ...
    süreç grubunu hedef almaya çalışır.
    """
    ...
    try:
        ...
        # Ardından, sistemde çalışan Minecraft.Windows.exe süreçlerini de
        # ad üzerinden sonlandırmaya çalış.
        try:
            subprocess.run(
                ["pkill", "-f", "Minecraft.Windows.exe"],
