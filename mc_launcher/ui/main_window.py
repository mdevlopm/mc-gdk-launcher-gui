...
class LauncherWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # ── Hakkında sayfası ────────────────────────────────────────────────
        about_group = Adw.PreferencesGroup(title="Minecraft GDK Linux Launcher")
        about_page.add(about_group)
        about_text = _t("msg_about")
        about_lbl = Gtk.Label(label=about_text)
        self.about_lbl = about_lbl
        about_lbl.set_halign(Gtk.Align.START)
        about_lbl.set_xalign(0.0)
        about_lbl.set_wrap(True)
        about_lbl.set_wrap_mode(Gtk.WrapMode.WORD_CHAR)
        about_lbl.set_selectable(True)
        about_lbl.set_margin_top(6)
        about_lbl.set_margin_bottom(6)
        about_group.add(about_lbl)
