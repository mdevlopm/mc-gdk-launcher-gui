...
def _do_extract(tar_path: str, on_status: Callable, remove_after: bool = False):
    ...
    try:
        ...
        with tarfile.open(tar_path, "r:gz") as t:
            ...
            total = len(members)
            dest_real = os.path.realpath(SCRIPT_DIR)
            for i, m in enumerate(members, 1):
                try:
                    try:
                        t.extract(m, SCRIPT_DIR, filter="tar")
                    except TypeError:
                        # Python < 3.12: manuel path-traversal kontrolü
                        member_path = os.path.realpath(os.path.join(SCRIPT_DIR, m.name))
        ...
        try:
            ...
            with urllib.request.urlopen(req2, timeout=120) as resp, open(tar_path, "wb") as out:
                ...
                while True:
                    ...
                    if total > 0:
                        on_status(f"İndiriliyor... {min(100, int(done * 100 / total))}%", "running")
