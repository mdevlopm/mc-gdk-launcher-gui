...
ICON_DEST_DIR_SVG="$HOME/.local/share/icons/hicolor/scalable/apps"
ICON_DEST_DIR_PNG="$HOME/.local/share/icons/hicolor/256x256/apps"
...
mkdir -p "$APP_DIR"
mkdir -p "$ICON_DEST_DIR_SVG"
mkdir -p "$ICON_DEST_DIR_PNG"

# Copy Icon
echo "[1/3] İkon kurulumu..."
ICON_NAME="com.mc.gdk.launcher"
# Eski isim veya formatı temizle
rm -f "$HOME/.local/share/icons/mc_gdk_launcher.png" "$HOME/.local/share/icons/mc_gdk_launcher.svg" "$HOME/.local/share/icons/${ICON_NAME}.png" "$HOME/.local/share/icons/${ICON_NAME}.svg"

if [ -f "$PROJECT_DIR/assets/minecraft_gdk_launcher_logo.svg" ]; then
    cp "$PROJECT_DIR/assets/minecraft_gdk_launcher_logo.svg" "$ICON_DEST_DIR_SVG/${ICON_NAME}.svg"
    # Execute python renderer to supply crisp raster fallbacks
    python3 "$PROJECT_DIR/render_icon.py" "$PROJECT_DIR/assets/minecraft_gdk_launcher_logo.svg" "$HOME/.local/share/icons/hicolor/" || true
elif [ -f "$PROJECT_DIR/assets/icon.svg" ]; then
    cp "$PROJECT_DIR/assets/icon.svg" "$ICON_DEST_DIR_SVG/${ICON_NAME}.svg"
    python3 "$PROJECT_DIR/render_icon.py" "$PROJECT_DIR/assets/icon.svg" "$HOME/.local/share/icons/hicolor/" || true
elif [ -f "$PROJECT_DIR/assets/icon.png" ]; then
    cp "$PROJECT_DIR/assets/icon.png" "$ICON_DEST_DIR_PNG/${ICON_NAME}.png"
else
    echo "WARNING: No icon found in assets directory. Continuing without custom icon."
